...
describe('PatientApi', () => {
    ...
    const createPatientWithDietOrder = async (
        name: string,
        minCal: number,
        maxCal: number
    ) => {
    ...
    describe('getAvailableMeals', () => {
        ...
        it('filters out recipes exceeding remaining calorie budget', async () => {
            ...
            const recipe_ids = result.recipes.map((m) => m.id);
            expect(recipe_ids).toContain(smallRecipe.id);
            expect(recipe_ids).not.toContain(hugeRecipe.id);
        });
        ...
        it('returns empty array when no recipes fit budget', async () => {
            ...
            await db.recipe.create({ data: { name: 'High Cal Only', category: 'Entrees', calories: 500 } });
